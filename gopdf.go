...
func (me *GoPdf) AddFontUnicode(family string, ttffile string) error {
	var ttfparser core.TTFParser
	err := ttfparser.Parse(ttffile)
	if err != nil {
		return err
	}
...
//TODO remove it
func DebugIObj(iobj IObj) {
	iobj.Build()
	fmt.Printf("%s\n", iobj.GetObjBuff().String())
}
